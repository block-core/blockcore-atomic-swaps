...
// Class definition
var DEXModalTwoFactorAuthentication = function () {
    // Private variables
    var modal;
    var modalObject;
			...
			if (smsValidator) {
				smsValidator.validate().then(function (status) {
					...
					if (status == 'Valid') {
						...
						smsSubmitButton.setAttribute('data-dex-indicator', 'on');
						...
						setTimeout(function() {
							...
							smsSubmitButton.removeAttribute('data-dex-indicator');
        ...
        appsSubmitButton.addEventListener('click', function (e) {
			...
			if (appsValidator) {
				appsValidator.validate().then(function (status) {
					...
					if (status == 'Valid') {
						appsSubmitButton.setAttribute('data-dex-indicator', 'on');

						// Disable button to avoid multiple click 
						appsSubmitButton.disabled = true;

						setTimeout(function() {
							appsSubmitButton.removeAttribute('data-dex-indicator');
    ...
    return {
        init: function () {
            // Elements
            modal = document.querySelector('#dex_modal_two_factor_authentication');
            ...
            optionsWrapper = modal.querySelector('[data-dex-element="options"]');
            optionsSelectButton = modal.querySelector('[data-dex-element="options-select"]');

            smsWrapper = modal.querySelector('[data-dex-element="sms"]');
            smsForm = modal.querySelector('[data-dex-element="sms-form"]');
            smsSubmitButton = modal.querySelector('[data-dex-element="sms-submit"]');
            smsCancelButton = modal.querySelector('[data-dex-element="sms-cancel"]');

            appsWrapper = modal.querySelector('[data-dex-element="apps"]');
            appsForm = modal.querySelector('[data-dex-element="apps-form"]');
            appsSubmitButton = modal.querySelector('[data-dex-element="apps-submit"]');
            appsCancelButton = modal.querySelector('[data-dex-element="apps-cancel"]');
