...
var DEXAuthI18nDemo = function() {
        ...
        for (var label in translationStrings) {
			if (translationStrings.hasOwnProperty(label)) {
				if (translationStrings[label][lang]) {
					let labelElement = document.querySelector('[data-dex-translate=' + label + ']');
	...
	var setLanguage = function(lang) {
		const selectedLang = menu.querySelector('[data-dex-lang="' + lang + '"]');

		if (selectedLang !== null) {
			const currentLangName = document.querySelector('[data-dex-element="current-lang-name"]'); 
			const currentLangFlag = document.querySelector('[data-dex-element="current-lang-flag"]'); 

			const selectedLangName = selectedLang.querySelector('[data-dex-element="lang-name"]');
			const selectedLangFlag = selectedLang.querySelector('[data-dex-element="lang-flag"]');

			currentLangName.innerText = selectedLangName.innerText;
			currentLangFlag.setAttribute("src", selectedLangFlag.getAttribute("src"));

			localStorage.setItem("dex_auth_lang", lang);
		}
	}

	var init = function() {
		if ( localStorage.getItem("dex_auth_lang") !== null ) {
			let lang = localStorage.getItem("dex_auth_lang");
			
			setLanguage(lang);
			translate(lang);
		}

		menuObj.on("DEX.menu.link.click", function(element) {
			let lang = element.getAttribute("data-dex-lang");
    ...
    return {
        // Initialization
        init: function() {
            menu = document.querySelector('#dex_auth_lang_menu');

			if (menu === null) {
				return;
			} 

			menuObj = DEXMenu.getInstance(menu);
