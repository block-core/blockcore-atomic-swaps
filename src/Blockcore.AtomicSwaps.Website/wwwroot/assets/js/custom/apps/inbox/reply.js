...
// Class definition
var DEXAppInboxReply = function () {

    // Private functions
    const handlePreviewText = () => {
        // Get all messages
        const accordions = document.querySelectorAll('[data-dex-inbox-message="message_wrapper"]');
        accordions.forEach(accordion => {
            // Set variables
            const header = accordion.querySelector('[data-dex-inbox-message="header"]');
            const previewText = accordion.querySelector('[data-dex-inbox-message="preview"]');
            const details = accordion.querySelector('[data-dex-inbox-message="details"]');
            const message = accordion.querySelector('[data-dex-inbox-message="message"]');
            ...
            header.addEventListener('click', e => {
                // Return if DEXMenu or buttons are clicked
                if (e.target.closest('[data-dex-menu-trigger="click"]') || e.target.closest('.btn')) {
                    return;
                } else {
    ...
    const initForm = () => {
        // Set variables
        const form = document.querySelector('#dex_inbox_reply_form');
        const allTagify = form.querySelectorAll('[data-dex-inbox-form="tagify"]');
    ...
    const handleCCandBCC = (el) => {
        // Get elements
        const ccElement = el.querySelector('[data-dex-inbox-form="cc"]');
        const ccButton = el.querySelector('[data-dex-inbox-form="cc_button"]');
        const ccClose = el.querySelector('[data-dex-inbox-form="cc_close"]');
        const bccElement = el.querySelector('[data-dex-inbox-form="bcc"]');
        const bccButton = el.querySelector('[data-dex-inbox-form="bcc_button"]');
        const bccClose = el.querySelector('[data-dex-inbox-form="bcc_close"]');
    ...
    const handleSubmit = (el) => {
        const submitButton = el.querySelector('[data-dex-inbox-form="send"]');
    ...
    // Init quill editor 
    const initQuill = (el) => {
        var quill = new Quill('#dex_inbox_form_editor', {
            modules: {
                toolbar: [
                    [{
                        header: [1, 2, false]
    ...
    const initDropzone = (el) => {
        ...
        const id = '[data-dex-inbox-form="dropzone"]';
        const dropzone = el.querySelector(id);
        const uploadButton = el.querySelector('[data-dex-inbox-form="dropzone_upload"]');
        ...
        var myDropzone = new Dropzone(id, { // Make the whole body a dropzone
            url: "https://preview.Blockcore.net/api/dropzone/void.php", // Set the url for your upload script location
            parallelUploads: 20,
